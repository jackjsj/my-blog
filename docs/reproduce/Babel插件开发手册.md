---
title: '【转载】Babel插件开发手册'
date: 2020/07/28 16:13:34
tags:
  - babel
  - 工程化
categories:
...
本文为转载，原文链接： [plugin-handbook.md](https://github.com/jamiebuilds/babel-handbook/blob/master/translations/zh-Hans/plugin-handbook.md)
...
> 如果您想要在 AST 节点属性中插入一个像`body </ 0>那样的数组。 它与 <code> insertBefore`/`insertAfter` 类似, 但您必须指定 `listKey` (通常是 `正文`).
...
编写转换时，通常需要构建一些要插入的节点进入 AST。 如前所述，您可以使用 `babel-types` 包中的 builder 方法。
...
That can be costly, because Babel does some processing each time a new
